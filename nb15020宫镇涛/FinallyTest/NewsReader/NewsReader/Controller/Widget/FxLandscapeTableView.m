...
@interface FxLandscapeTableView (FxLandscapeTableViewPrivate) <UIScrollViewDelegate>

- (void)configureCells;
- (void)configureCell:(FxLandscapeCell *)cell forIndex:(NSInteger)index;

- (void)recycleCell:(FxLandscapeCell *)cell;
...
@implementation FxLandscapeTableView
...
- (void)reloadData
{
    ...
    // recycle all cells
    for (FxLandscapeCell *cell in _visibleCells) {
...
{
    FxLandscapeCell *result = [_recycledCells anyObject];
...
- (void)configureCells
{
    ...
    for (NSInteger index = firstCell; index <= lastCell; index++) {
        ...
        if ([self cellForIndex:index] == nil) {
            ...
            FxLandscapeCell *cell = [_dataSource cellInTableView:self atIndex:index];
...
- (void)didEndMoving
{
    if (_scrollViewIsMoving) {
        ...
        if (_lastCellIndex != _currentCellIndex) {
            ...
            FxLandscapeCell *cell = [self cellForIndex:_lastCellIndex];
